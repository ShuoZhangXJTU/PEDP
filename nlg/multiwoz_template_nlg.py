...
class MultiwozTemplateNLG():
    def __init__(self, is_user, args):
        """
        ...
        self.manual_user_template = read_json(os.path.join(args.nlg_dir, 'manual_user_template_nlg.json'))
        self.manual_system_template = read_json(os.path.join(args.nlg_dir, 'manual_system_template_nlg.json'))
    ...
    def _manual_generate(self, dialog_acts, template):
        ...
        # dialog_acts = dict()
        # for domain_, intent_, slot_, val_ in dialog_acts_in:
        #     if domain_ not in ['general', 'none']:
        #         domain_ = domain_.title()
        #     if intent_ not in ['general', 'none']:
        #         intent_ = intent_.title()
        #     if slot_ not in ['general', 'none']:
        #         slot_ = slot_.title()
        #     DI = domain_ + '-' + intent_
        #     SV = [slot_, val_]
        #     if DI not in dialog_acts:
        #         dialog_acts[DI] = [SV]
        #     else:
        #         dialog_acts[DI].append(SV)
        #
        # import ipdb;
        # ipdb.set_trace()
